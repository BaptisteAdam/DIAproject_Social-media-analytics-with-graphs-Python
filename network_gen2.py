...
def adjency_matrix(my_network, list_nodes, weights):
    ...
    for node in list_nodes : 
        if list_nodes[node]['sex'] == 'Male' or list_nodes[node]['sex'] == 'Female' :
            list_left_nodes.append([node, list_nodes[node]['sex'] , list_nodes[node]['age'], list_nodes[node]['daily_use']])
        elif list_nodes[node]['sex'] == 'Genderqueer/Non-Binary' or copy_nx.nodes[node]['sex'] == 'NaN' :
            list_right_nodes.append([node, list_nodes[node]['sex'] , list_nodes[node]['age'], list_nodes[node]['daily_use']])
    ...
    for i, nodeL in enumerate(list_left_nodes) :
        for j, nodeR in enumerate(list_right_nodes) :
            ...
            adjency_matrix[i, j] = cte*matching_pb #the value of the edge between the two nodes is added to the adjency matrix
...
if __name__ == "__main__":
    ...
    nodes_matching_pb = side_nodes(my_network, 3, "Male")
    
    #print(adjency_matrix(my_network, nodes_matching_pb, weights))
